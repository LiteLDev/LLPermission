...
PERMAPI void PERM_CreateRole(const std::string& name, const std::string& displayName, std::weak_ptr<ll::perm::Role>& ret) {
...
PERMAPI void PERM_GetRole(const std::string& name, std::weak_ptr<ll::perm::Role>& ret) {
    ret = mod.perm.getRole(name);
}

PERMAPI void PERM_GetOrCreateRole(const std::string& name, std::weak_ptr<ll::perm::Role>& ret) {
...
PERMAPI void PERM_GetPlayer$1bilities(const xuid_t& xuid, ll::perm::Permissions& ret) {
    ret = mod.perm.getPlayerPermissions(xuid);
}

PERMAPI void PERM_GetPlayerRoles(const xuid_t& xuid, ll::perm::Roles& ret) {
